...
def test_sinkhorn_dimensional_consistency(entropy):
    ...
    sinkhorn_asym(a, x, b, y, p=2, entropy=entropy)
...
def test_sinkhorn_asym_infinite_blur(entropy):
    ...
    f, g = sinkhorn_asym(a, x, b, y, p=2, entropy=entropy, nits=10000, tol=1e-10)
    assert torch.allclose(f, f_c, 1e-4)
    assert torch.allclose(g, g_c, 1e-4)
...
def test_sinkhorn_sym_infinite_blur(entropy):
    ...
    _, f = sinkhorn_sym(a, x, p=p, entropy=entropy, nits=50000, tol=1e-10)
    assert torch.allclose(f, f_c, 1e-4)


@pytest.mark.parametrize('entropy', [KullbackLeibler(1e0, 1e0), Balanced(1e0), TotalVariation(1e0, 1e0),
                                     Range(1e0, 0.3, 2), PowerEntropy(1e0, 1e0, 0), PowerEntropy(1e0,1e0, -1)])
def test_sinkhorn_consistency_sym_asym(entropy):
    p=2
    a, x = generate_measure(3, 5, 2)
    f_a, g_a = sinkhorn_asym(a, x, a, x, p=p, entropy=entropy)
    _, f_s = sinkhorn_sym(a, x, p=p, entropy=entropy, nits=10000, tol=1e-10)
