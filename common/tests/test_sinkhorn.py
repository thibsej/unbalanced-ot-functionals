...
def test_sinkhorn_asym_infinite_blur_unbalanced(solv, entropy, atol, p, m, n, reach):
    entropy.reach = reach
    ...
    assert torch.allclose(torch.tensor([0.]), entropy.error_sink(f, f_c), atol=atol)
    assert torch.allclose(torch.tensor([0.]), entropy.error_sink(g, g_c), atol=atol)
...
@pytest.mark.parametrize('solv', [BatchVanillaSinkhorn(nits=10000, nits_grad=10, tol=1e-14, assume_convergence=True),
                                  BatchScalingSinkhorn(budget=1000, nits_grad=10, assume_convergence=True),
                                  BatchExpSinkhorn(nits=10000, nits_grad=10, tol=1e-14, assume_convergence=True)])
def test_sinkhorn_asym_infinite_blur_balanced(solv, entropy, atol, p, m, reach):
    entropy.reach = reach
    cost = euclidean_cost(p)
    a, x = generate_measure(2, 5, 3)
    b, y = generate_measure(2, 6, 3)
    ...
    assert torch.allclose(entropy.error_sink(f, f_c), torch.tensor([0.]), atol=atol)
    assert torch.allclose(entropy.error_sink(g, g_c), torch.tensor([0.]), atol=atol)
...
@pytest.mark.parametrize('reach', [0.5, 1., 2.])
@pytest.mark.parametrize('m', [0.7, 1., 2.])
@pytest.mark.parametrize('entropy,atol', [(KullbackLeibler(1e0, 1e0), 1e-6), (Balanced(1e0), 1e-6),
...
def test_sinkhorn_consistency_sym_asym(solv, entropy, atol, p, m, reach):
    ...
    cost = euclidean_cost(p)
    a, x = generate_measure(2, 5, 3)
    f_a, g_a = solv.sinkhorn_asym(m * a, x, m * a, x, cost=cost, entropy=entropy)
    _, f_s = solv.sinkhorn_sym(m * a, x, cost=cost, entropy=entropy)
    assert torch.allclose(entropy.error_sink(f_a, f_s), torch.tensor([0.]), atol=atol)
    assert torch.allclose(entropy.error_sink(g_a, f_s), torch.tensor([0.]), atol=atol)
...
def test_sinkhorn_consistency_exp_log_asym(entropy, rtol, p, m, reach):
    ...
    f_a, g_a = solver1.sinkhorn_asym(m * a, x, m * b, y, cost=cost, entropy=entropy)
    u_a, v_a = solver2.sinkhorn_asym(m * a, x, m * b, y, cost=cost, entropy=entropy)
...
def test_sanity_control_exp_sinkhorn_small(entropy):
    ...
    f, g = solver.sinkhorn_asym(a, x, b, y, cost=euclidean_cost(1), entropy=entropy)
