...
def regularized_ot(a, x, b, y, p, entropy, nits=100, tol=1e-3, assume_convergence=False):  # OT_eps
    g_y, f_x = sinkhorn_asym(a, x, b, y, p=p, entropy=entropy, nits=nits, tol=tol, assume_convergence=assume_convergence)
...
def hausdorff_divergence(a, x, b, y, p, entropy, nits=100, tol=1e-3, assume_convergence=False):  # H_eps
    g_xy, f_x = sinkhorn_sym(a, x, p=p, entropy=entropy, y_j=y, nits=nits, tol=tol, assume_convergence=assume_convergence)
    f_yx, g_y = sinkhorn_sym(b, y, p=p, entropy=entropy, y_j=x, nits=nits, tol=tol, assume_convergence=assume_convergence)
...
def sinkhorn_divergence(a, x, b, y, p, entropy, nits=100, tol=1e-3, assume_convergence=False):  # S_eps
    g_yx, f_xy = sinkhorn_asym(a, x, b, y, p=p, entropy=entropy, nits=nits, tol=tol, assume_convergence=assume_convergence)
