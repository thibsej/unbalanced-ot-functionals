...
def gradient_flow(func, sig=0.05, p=1.5, lr_x=.05, lr_a=0.0,
    ...
    if func is energyDistance:
        loss = partial(func, p=p)
    if func is gaussianKernel:
        loss = partial(func, sig=sig)
    path = path + f"/unbalanced_flow_{func.__name__}_p{p}" \
...
if __name__ == '__main__':
    setting = 2
    p, sig = 2, euclidean_cost(2)

    # Compute KL for a smaller blur to compare with previous
    # higher regularization
    if setting == 1:
        gradient_flow(gaussianKernel, sig=0.1, p=1.5,
                      lr_x=60., lr_a=0.0, Nsteps=300)
        gradient_flow(energyDistance, sig=0.1, p=1.5,
                      lr_x=60., lr_a=0.0, Nsteps=300)

    if setting == 2:
        gradient_flow(gaussianKernel, sig=0.1, p=1.5,
                      lr_x=60., lr_a=1.0, Nsteps=300)
