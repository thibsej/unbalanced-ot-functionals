...
def display_samples(ax, x, a, color):
    ...
    ax.scatter( x_[:,0], x_[:,1], s=200*100*a_, c=color, edgecolors='none' )
...
def gradient_flow(func, entropy, solver, p, lr_x=.05, lr_a=.005, Nsteps=100, path=path):
    """
    ...
    path = path + f"/unbalanced_flow_{func.__name__}_p{p}_{entropy.__dict__}_lrx{lr_x}_lra{lr_a}_steps{Nsteps}"
    if not os.path.isdir(path):
        os.mkdir(path)
